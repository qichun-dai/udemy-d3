...
d3.json("data/unemployment.json").then(data => {    
	data.forEach(d => {
		d.call_revenue = Number(d.unemployment)
	...
	revenueBar = new BarChart("#revenue", "call_revenue", "Monthly Unemployment Rate %")
	//durationBar = new BarChart("#call-duration", "call_duration", "Average call duration (seconds)")
	//unitBar = new BarChart("#units-sold", "units_sold", "Units sld per call")
